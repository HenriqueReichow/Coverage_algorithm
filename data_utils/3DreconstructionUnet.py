...
#                 for t in range(theta):
#                     for r in range(radius):
#                         if matrix[r][t]>0 and matrix[r][t]<5:
#                             #print("aqio")
...
# output_xyz_filepath=(f"experiments-unet/obj_0_pointcloud/p900.xyz")
# config={
#                     "RangeBins": 256,
#                     "AzimuthBins": 192,
#                     "RangeMin": 0,
...
#     auv_metadata_dir = "/home/lh/Desktop/Coverage_algorithm/Sonar-Dataset-mission-1-obj0/1-sphere-0-data/Meta-data/"
...
def matrix2xyz(matrix,output_xyz_filepath,index,mission,auv,mission_metadata):
        sonar_configuration = json.load(open('sonar-configuration.json'))
        sonar_model=sonar_configuration["P900"]
        auv_metadata=json.load(open(f"/home/lh/Desktop/Coverage_algorithm/Sonar-Dataset-mission-1-obj0/1-sphere-0-data/Meta-data/{index}.json"))
        radius, theta = matrix.shape
        #print(matrix.shape)
        try:
            with open(output_xyz_filepath, 'a') as outfile:
...
output_xyz_filepath=(f"experiments-unet/{m}-auvT-{0}.xyz")
#image_file_path = "teste.png"
for i in tqdm.tqdm(range(101)):
    image_file_path = (f"/home/lh/Desktop/Coverage_algorithm/experiments-unet/obj-0/{i}.png")
    matrix2xyz(matrix=png2matrix(image_file_path),output_xyz_filepath=output_xyz_filepath,index=i,mission=m,auv=0,mission_metadata=mission)
# Example usage:
